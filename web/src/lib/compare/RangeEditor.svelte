<script lang="ts">
    ...
    //Moving the window a plan asks about, so a fresh round of dates covers different days
    ...
    */
    ...
    //Opened on what the plan asks about now, so a shut form never keeps a range nobody saved
    function open() {
        panel.show();
        newStart = start;
        newEnd = end;
...
<div class="extend" class:wide={panel.open || Boolean(panel.msg)}>
    {#if !panel.open}
        <button class="ghost" onclick={open}>Change the date range</button>
    {:else}
